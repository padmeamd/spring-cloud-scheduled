 ...
 *
 * <p>
 * This enables detection of {@link CloudScheduled @Scheduled} annotations on
 * any
 ...
 *
 * <p>
 * Methods annotated with {@code @Scheduled} may even be declared directly
 * within
 ...
 * a unique {@link org.springframework.scheduling.TaskScheduler} bean in the
 * context,
 * or a {@code TaskScheduler} bean named "taskScheduler" otherwise; the same
 * lookup
 * will also be performed for a
 ...
 *
 * <p>
 * The examples are equivalent save that in XML a <em>fixed-rate</em> period is
 * used
 * instead of a custom <em>{@code Trigger}</em> implementation; this is because
 * the
 * {@code task:} namespace {@code scheduled} cannot easily expose such support.
 * This is
 * but one demonstration how the code-based approach allows for maximum
 * configurability
 * through direct access to the actual component.
 *
 * <p>
 * <b>Note: {@code @EnableScheduling} applies to its local application context
 * only,
 * allowing for selective scheduling of beans at different levels.</b> Please
 * redeclare
 * {@code @EnableScheduling} in each individual context, for example, the common
 * root web
 * application context and any separate {@code DispatcherServlet} application
 * contexts,
 ...
 * @see Trigger
 * @see CloudScheduledAnnotationBeanPostProcessor
 */
