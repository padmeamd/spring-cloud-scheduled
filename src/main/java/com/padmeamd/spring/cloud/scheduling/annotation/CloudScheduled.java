...
package com.padmeamd.spring.cloud.scheduling.annotation;
 ...
 * For a one-time task, it is sufficient to just specify an
 * {@link #initialDelay}.
 *
 * <p>
 ...
 *
 * <p>
 * Methods that return a reactive {@code Publisher} or a type which can be
 * adapted
 * to {@code Publisher} by the default {@code ReactiveAdapterRegistry} are
 * supported.
 * The {@code Publisher} must support multiple subsequent subscriptions. The
 * returned
 * {@code Publisher} is only produced once, and the scheduling infrastructure
 * then
 ...
 *
 * <p>
 * Kotlin suspending functions are also supported, provided the
 * coroutine-reactor
 * bridge ({@code kotlinx.coroutine.reactor}) is present at runtime. This bridge
 * is
 ...
 *
 * <p>
 * This annotation can be used as a <em>{@linkplain Repeatable repeatable}</em>
...
@Documented
@Reflective
public @interface CloudScheduled {
	 ...
	 * <li>any of the known
	 * {@link org.springframework.format.annotation.DurationFormat.Style
	 * DurationFormat.Style}: the
	 * {@link org.springframework.format.annotation.DurationFormat.Style#ISO8601
	 * ISO8601}
	 * style or the
	 * {@link org.springframework.format.annotation.DurationFormat.Style#SIMPLE
	 * SIMPLE} style
	 * &mdash; using the {@link #timeUnit()} as fallback if the string doesn't
	 * contain an explicit unit</li>
	 * <li>one of the above, with Spring-style "${...}" placeholders as well as SpEL
	 * expressions</li>
	 * </ul>
	 * 
	 ...
	 *         or a
	 *         {@link org.springframework.format.annotation.DurationFormat.Style#ISO8601
	 *         java.time.Duration} compliant value
	 *         or a
	 *         {@link org.springframework.format.annotation.DurationFormat.Style#SIMPLE
	 *         simple format} compliant value
	 * @since 3.2.2
	 * @see #fixedRate()
	 */
	 ...
	 * {@link #timeUnit}.
	 * <p>
	 * <b>NOTE: With virtual threads, fixed rates and cron triggers are recommended
	 * over fixed delays.</b> Fixed-delay tasks operate on a single scheduler thread
	 * with
	 * {@link org.springframework.scheduling.concurrent.SimpleAsyncTaskScheduler}.
	 * 
	 * @return the delay
	 */
	 ...
	 * </ul>
	 * 
	 * @return the initial delay as a String value &mdash; for example a
	 *         placeholder,
	 *         or a
	 *         {@link org.springframework.format.annotation.DurationFormat.Style#ISO8601
	 *         java.time.Duration} compliant value
	 ...
	 * <p>
	 * This attribute is ignored for {@linkplain #cron() cron expressions}
	 * and for {@link java.time.Duration} values supplied via
	 * {@link #fixedDelayString},
	 ...
	 * <p>
	 * May be used to determine the target scheduler to be used,
